...
*/
    ...
    private StringBuffer createEDIText(OLEBatchProcessProfileBo oleBatchProcessProfileBo, PurchaseOrderDocument purchaseOrderDocument, List<OlePurchaseOrderItem> purchaseOrderItemList) {
        ...
        String instSanNum = OLEConstants.getParameter(OLEConstants.APPL_ID_OLE, OLEConstants.SELECT_NMSPC, OLEConstants.SELECT_CMPNT, OLEConstants.EDIBatchProfile.INST_SAN_NUM);
        ...
        for (OLEBatchProcessProfileConstantsBo oleBatchProcessProfileConstantsBo : oleBatchProcessProfileConstantsList) {
            ...
            if (oleBatchProcessProfileConstantsBo.getAttributeName().equalsIgnoreCase(OLEConstants.EDIBatchProfile.INTERCHANGE_DETAILS)) {
                ...
                ediValues.put(OLEConstants.EDIBatchProfile.INTERCHANGE_DETAILS, splitString[0] + instSanNum + splitString[1] + purchaseOrderDocument.getVendorDetail().getVendorRemitName() + splitString[2] + dateFormatter.format(new Date()) + splitString[3] + randomNumber + splitString[4]);
        ...
        for (OlePurchaseOrderItem purchaseOrderItem : purchaseOrderItemList) {
            ...
            for (OLEBatchProcessProfileConstantsBo oleBatchProcessProfileConstantsBo : oleBatchProcessProfileConstantsList) {
                ...
                if (oleBatchProcessProfileConstantsBo.getAttributeName().equalsIgnoreCase(OLEConstants.EDIBatchProfile.PURCHASE_LINE_ITEM)) {
                    try{
                        ediItemValues.put(OLEConstants.EDIBatchProfile.PURCHASE_LINE_ITEM, oleBatchProcessProfileConstantsBo.getAttributeValue().replace(OLEConstants.EDIBatchProfile.SYMBOL_ASTERISK, isbn!=null && !isbn.equalsIgnoreCase("") ? isbnUtil.normalizeISBN(isbn) : ""));
                    }catch(OleException oe){
                        LOG.error("SaveEDi - Invalid ISBN" +oe.getMessage(), oe);
                    }
                    lineCount++;
                ...
                if (oleBatchProcessProfileConstantsBo.getAttributeName().equalsIgnoreCase(OLEConstants.OlePatronBill.LABEL_NOTE)) {
                    ...
                    int i=0;
                    if (notesList.size() > 0) {
                        for(OleNotes notes:notesList) {
                            if (notes.getNoteType().getNoteType().equalsIgnoreCase("Vendor Instructions Note")) {
                                vendorNote = vendorNote+notesList.get(i).getNote()+". ";
                            } i++;
                        }ediItemValues.put(OLEConstants.OlePatronBill.LABEL_NOTE, oleBatchProcessProfileConstantsBo.getAttributeValue().replace(OLEConstants.EDIBatchProfile.SYMBOL_ASTERISK,vendorNote));
                    }
                    lineCount++;
                }
                if (oleBatchProcessProfileConstantsBo.getAttributeName().equalsIgnoreCase(OLEConstants.EDIBatchProfile.DELIVERY_ADDRESS)) {
                    ...
                    ediItemValues.put(OLEConstants.EDIBatchProfile.DELIVERY_ADDRESS, (address[0] + instSanNum + address[1] + getDeliveryDetails(purchaseOrderDocument) + address[2]));
                    lineCount++;
                }
                if (oleBatchProcessProfileConstantsBo.getAttributeName().equalsIgnoreCase(OLEConstants.EDIBatchProfile.OTHER_ADDRESS)) {
                    String[] address = oleBatchProcessProfileConstantsBo.getAttributeValue().split("\\*");
                    ediItemValues.put(OLEConstants.EDIBatchProfile.OTHER_ADDRESS, (address[0] + instSanNum + address[1] + getDeliveryDetails(purchaseOrderDocument) + address[2]));
                    lineCount++;
                }
